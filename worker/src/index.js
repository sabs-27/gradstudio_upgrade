...
import {
  ...
  handleGetCarouselCardCourses,
  handleMigrateCarouselCourseLinks
} from './handlers/adminCarousel.js';


async function requireAdmin(request, env) {
  // Auth disabled for admin panel - all admin routes are open
  return { role: 'admin' };
}
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      try {
        ...
        if (path.match(/^\/api\/courses\/[a-zA-Z0-9_-]+$/)) {
        ...
        if (path.match(/^\/api\/carousel-card\/[0-9]+\/courses$/) && request.method === 'GET') {
          const cardId = path.split('/')[3];
          return await handleGetCarouselCardCourses(cardId, env, corsHeaders);
        }
        ...
        // MIGRATION: Add parent_course_id and type columns if missing
        if (path === '/api/admin/migrate' && request.method === 'POST') {
          ...
          await env.DB.prepare(`ALTER TABLE courses ADD COLUMN type TEXT DEFAULT 'course'`).run().catch(() => { });
          // Set existing courses with children as subcategories
          await env.DB.prepare(`UPDATE courses SET type = 'subcategory' WHERE id IN (SELECT DISTINCT parent_course_id FROM courses WHERE parent_course_id IS NOT NULL)`).run().catch(() => { });
        ...
        const adminPrefix = env.ADMIN_PREFIX || 'admin-preview/';

        // Serve admin preview
        if (path === '/admin-preview' || path === '/admin-preview/') {
          const r2Key = adminPrefix + 'index.html';
          const object = await env.R2.get(r2Key);
          if (!object) return new Response('Not found', { status: 404, headers: corsHeaders });
          let html = await object.text();
          // Runtime dedup: remove duplicate function definitions that got stuck in R2 cache
          const dupes = ['handleAddCourse', 'openEditCourseModal', 'handleEditCourse'];
          for (const fname of dupes) {
            const re = new RegExp('(async\\s+)?function\\s+' + fname, 'g');
            const matches = [...html.matchAll(re)];
            if (matches.length > 1) {
              // Keep the first, remove the second occurrence (and its function body)
              const idx2 = matches[1].index;
              let start = html.lastIndexOf('\n', idx2);
              let depth = 0, end = idx2;
              for (let i = idx2; i < html.length; i++) {
                if (html[i] === '{') depth++;
        ...
        if (path.startsWith('/admin-preview/')) {
          let adminFilename = path.slice('/admin-preview/'.length);
          if (adminFilename === '') adminFilename = 'index.html';
          return await serveStaticFromR2(adminPrefix + adminFilename, env, corsHeaders);
        }

        // Friendly URL aliases
        const ALIASES = {
          '/': staticPrefix + 'index.html',
          '/about': staticPrefix + 'about.html',
          '/contact': staticPrefix + 'contact.html',
          '/legal/privacy': staticPrefix + 'privacy.html',
          '/legal/cookies': staticPrefix + 'cookies.html',
          '/legal/cookie-preferences': staticPrefix + 'cookie-preferences.html',
          '/legal/terms': staticPrefix + 'terms.html',
        };
        ...
        // For any non-API path, try to serve from R2 as staticPrefix + <filename>
        let filename = path.startsWith('/') ? path.slice(1) : path;
        if (filename === '') filename = 'index.html';

        if (filename && !filename.includes('..')) {
          let testKey = staticPrefix + filename;
          let obj = await env.R2.get(testKey);
          if (!obj && staticPrefix !== 'static/') {
            testKey = 'static/' + filename;
            obj = await env.R2.get(testKey);
          }
          if (obj) {
            return await serveStaticFromR2(testKey, env, corsHeaders);
          }
        }

        // Fallback for subdirectories (e.g. /login -> /login.html)
        if (filename && !filename.includes('.') && !filename.includes('..')) {
          let testKey = staticPrefix + filename + '.html';
          let objHtml = await env.R2.get(testKey);
          if (!objHtml && staticPrefix !== 'static/') {
            testKey = 'static/' + filename + '.html';
            objHtml = await env.R2.get(testKey);
          }
          if (objHtml) {
            return await serveStaticFromR2(testKey, env, corsHeaders);
          }
...
async function serveStaticFromR2(r2Key, env, corsHeaders) {
  ...
  // Fallback to production static/ if running in dev and object not found
  if (!object && env.STATIC_PREFIX && env.STATIC_PREFIX !== 'static/' && r2Key.startsWith(env.STATIC_PREFIX)) {
    const fallbackKey = r2Key.replace(env.STATIC_PREFIX, 'static/');
  ...
  // Smart caching: HTML = no-cache (updates cache-bust params),
  // JS/CSS/images = cached (loaded with ?v= cache-busting params)
  const isHtml = ext === '.html';
  let cacheControl;
  if (isHtml) {
    cacheControl = 'no-cache, no-store, must-revalidate';
  } else {
    // 1 hour browser cache, 7 day CDN cache — safe because JS/CSS use ?v= params
    cacheControl = 'public, max-age=3600, s-maxage=604800';
  }
...
async function handleGetAllCourses(request, env, corsHeaders) {
  ...
  // Query D1 database for courses with counts + parent_course_id
  let results;
  ...
  try {
    ...
    if (!err.message.includes('parent_course_id') && !err.message.includes('type')) throw err;
  ...
  const allCourses = results.map(row => ({
    ...
    type: row.type || (row.parent_course_id ? 'course' : 'subcategory'), // Fallback logic
...
async function handleGetCourse(courseId, request, env, corsHeaders) {
  ...
  if (results.length === 0) {
    ...
    const courseCheck = await env.DB.prepare(`
      SELECT id, title, icon_class, description, parent_course_id FROM courses WHERE id = ? AND is_active = 1 AND is_deleted = 0
    `).bind(courseId).first();
    ...
    if (courseCheck) {
      ...
      const emptyResponse = {
        ...
        auto_number: !courseCheck.parent_course_id,
        sections: [],
        totalItems: 0
      };
  ...
  // Check if course has parent_course_id to determine auto_number
  let autoNumber = true;
  try {
    const courseRow = await env.DB.prepare('SELECT parent_course_id FROM courses WHERE id = ?').bind(courseId).first();
    if (courseRow && courseRow.parent_course_id) autoNumber = false;
  } catch (_) { }
...
async function handleSearch(query, courseId, request, env, corsHeaders) {
  ...
  let sql = `
    ...
    JOIN courses c ON sim.course_id = c.id AND c.is_deleted = 0 AND c.is_active = 1
...
async function handleAdminAddCourse(data, env, corsHeaders) {
  ...
  try {
    // Auto-create Main sub-course when adding the first sub-course to a parent with existing sections
    let autoCreatedMain = null;
    if (parent_course_id) {
      // Check if parent has existing sub-courses
      const existingSubs = await env.DB.prepare(
        'SELECT COUNT(*) as cnt FROM courses WHERE parent_course_id = ?'
      ).bind(parent_course_id).first();

      if (!existingSubs || existingSubs.cnt === 0) {
        // Check if parent has existing sections
        const existingSections = await env.DB.prepare(
          'SELECT COUNT(*) as cnt FROM sections WHERE course_id = ? AND is_deleted = 0'
        ).bind(parent_course_id).first();

        if (existingSections && existingSections.cnt > 0) {
          // Get parent course info
          const parentCourse = await env.DB.prepare(
            'SELECT title, icon_class, category FROM courses WHERE id = ?'
          ).bind(parent_course_id).first();

          const mainId = parent_course_id + '_main';
          const mainTitle = (parentCourse ? parentCourse.title : parent_course_id) + ' Main';
          const mainIcon = parentCourse ? parentCourse.icon_class : 'fas fa-book';
          const mainCategory = parentCourse ? parentCourse.category : category;

          // Create the Main sub-course
          try {
            await env.DB.prepare(`
              INSERT INTO courses (id, title, icon_class, category, display_order, is_active, parent_course_id, type)
              VALUES (?, ?, ?, ?, 0, 1, ?, 'course')
            `).bind(mainId, mainTitle, mainIcon, mainCategory, parent_course_id).run();
          } catch (insertErr) {
            if (insertErr.message && insertErr.message.includes('no such column')) {
              await env.DB.prepare(`
                ...
                VALUES (?, ?, ?, ?, 0, 1)
              `).bind(mainId, mainTitle, mainIcon, mainCategory).run();
            } else {
              throw insertErr;
            }
          }

          // Move all existing sections from parent to the Main sub-course
          await env.DB.prepare(
            'UPDATE sections SET course_id = ? WHERE course_id = ?'
          ).bind(mainId, parent_course_id).run();

          // Also move simulations that reference the parent course
          await env.DB.prepare(
            'UPDATE simulations SET course_id = ? WHERE course_id = ?'
          ).bind(mainId, parent_course_id).run();

          autoCreatedMain = mainId;
          console.log(`Auto-created Main sub-course '${mainId}' and moved sections from '${parent_course_id}'`);
        }
      }
    }
    ...
    if (parent_course_id) await env.KV.delete(`metadata:${parent_course_id}`);
    if (autoCreatedMain) await env.KV.delete(`metadata:${autoCreatedMain}`);

    return jsonResponse({ success: true, id, autoCreatedMain }, 201, corsHeaders);
  } catch (err) {
    // Robust retry for missing columns
    if (err.message && (err.message.includes('no such column: type') || err.message.includes('no such column: parent_course_id'))) {
...
async function handleAdminDeleteCourse(id, env, corsHeaders) {
  ...
  try {
    ...
    // AUTO-MERGE: If this was a sub-course and the parent now only has _main left,
    // merge _main back into the parent (move sections + simulations, delete _main)
    try {
      const deletedCourse = await env.DB.prepare('SELECT parent_course_id FROM courses WHERE id = ?').bind(id).first();
      if (deletedCourse && deletedCourse.parent_course_id) {
        const parentId = deletedCourse.parent_course_id;
        // Count remaining active (non-deleted) sub-courses under this parent
        const remaining = await env.DB.prepare(
          'SELECT id FROM courses WHERE parent_course_id = ? AND is_deleted = 0'
        ).bind(parentId).all();
        const remainingIds = remaining.results.map(r => r.id);
        const mainId = parentId + '_main';
        // If only _main remains (or nothing remains), merge _main back
        if (remainingIds.length === 0 || (remainingIds.length === 1 && remainingIds[0] === mainId)) {
          // Move sections and simulations from _main back to parent
          await env.DB.prepare('UPDATE sections SET course_id = ? WHERE course_id = ?').bind(parentId, mainId).run();
          await env.DB.prepare('UPDATE simulations SET course_id = ? WHERE course_id = ?').bind(parentId, mainId).run();
          // Delete the _main course record
          await env.DB.prepare('DELETE FROM courses WHERE id = ?').bind(mainId).run();
        }
      }
    } catch (mergeErr) {
      console.error('Auto-merge _main error (non-fatal):', mergeErr);
    }
...
async function handleAdminUpdateCourse(data, env, corsHeaders) {
  ...
  try {
    ...
    let autoCreatedMain = null;
    if (parent_course_id) {
      // Check if parent has existing sub-courses (excluding the course being moved)
      const existingSubs = await env.DB.prepare(
        'SELECT COUNT(*) as cnt FROM courses WHERE parent_course_id = ? AND id != ?'
      ).bind(parent_course_id, id).first();
      ...
      if (!existingSubs || existingSubs.cnt === 0) {
        ...
        if (existingSections && existingSections.cnt > 0) {
          ...
          // Create the Main sub-course (skip if already exists)
          const existing = await env.DB.prepare('SELECT id FROM courses WHERE id = ?').bind(mainId).first();
          if (!existing) {
            try {
              await env.DB.prepare(`
                INSERT INTO courses (id, title, icon_class, category, display_order, is_active, parent_course_id, type)
    ...
    if (parent_course_id) await env.KV.delete(`metadata:${parent_course_id}`);
    if (autoCreatedMain) await env.KV.delete(`metadata:${autoCreatedMain}`);
    return jsonResponse({ success: true, autoCreatedMain }, 200, corsHeaders);
  } catch (err) {
