...
function rendercourseview(coursedata, containerid, title, iconclass, courseid, sectionId = null) {
    ...
    sections.forEach((section, sidx) => {
        ...
        // section header - auto-number for legacy courses, skip for new admin-created courses
        const autoNumber = coursedata.auto_number !== false;
        const hasOwnSectionNum = /^\d+[\.\-\:\s]/.test(sectiontitle.trim());
        const displaySectionTitle = (!autoNumber || hasOwnSectionNum) ? sectiontitle : `${sectionnumber}: ${sectiontitle}`;
        chapterhtml += `
        ...
        items.forEach((item, iidx) => {
            ...
            // Auto-number badge for legacy courses, skip for new admin-created courses
            const hasOwnNumbering = /^\d+[\.\-]/.test(itemName.trim());
            const badgeHtml = (!autoNumber || hasOwnNumbering) ? '' : `<span class="chapter-badge">${indexstr}</span>`;
    ...
    await Promise.all(courseIds.map(async (courseId) => {
        try {
            ...
            if (courseData) {
                ...
                // Fix: use actual course title from API instead of ID fallback
                if (grouped[courseId] && (courseData.title || courseData.name)) {
                    grouped[courseId].courseTitle = courseData.title || courseData.name;
                }
                if (grouped[courseId] && courseData.icon_class) {
                    grouped[courseId].iconClass = courseData.icon_class;
                }
            }
        } catch (error) {
...
async function switchprovider(provider, callback, opts = {}) {
    ...
    // Handle course_page target type - opens dedicated course listing page
    if (targetType === 'course_page') {
        const cardId = opts.card_id || provider;
        const cardTitle = opts.card_title || '';
        const cardIcon = opts.card_icon || 'fa-solid fa-book';
        const cardColor = opts.card_color || '#3b82f6';
        openCarouselCoursePage(cardId, cardTitle, cardIcon, cardColor);
    ...
    if (targetType === 'course_sidebar' || targetType === 'category_sidebar') {
        ...
        setTimeout(async () => {
            const megaMenu = document.getElementById('sidebar-mega-menu-container');
            if (!megaMenu) return;

            // Get all sidebar items in column 1 (categories)
            const col1Items = megaMenu.querySelectorAll('.sidebar-column:first-child .sidebar-mega-item');

            if (targetType === 'category_sidebar') {
                const categories = window.categories || [];
                const cat = categories.find(c => String(c.id).toLowerCase() === String(provider).toLowerCase());
                const catName = cat ? cat.name : provider;

                col1Items.forEach(item => {
                    if (item.textContent.trim() === catName) item.click();
                });
            } else {
                // course_sidebar: find the course's category, click it, then click the course
                const courses = window.courses || [];
                let categories = window.categories || [];
                if (categories.length === 0) {
                    try {
                        const res = await fetch(API_CONFIG.baseUrl + '/api/categories?t=' + Date.now());
                        const data = await res.json();
                        categories = data.categories || [];
                        window.categories = categories;
                    } catch (e) { /* silently fail */ }
                }
                const course = courses.find(c => String(c.id).toLowerCase() === String(provider).toLowerCase());
                if (course) {
                    let parentId = course.parent_course_id;
                    const hasParentCourse = parentId && String(parentId).toLowerCase() !== 'null' && String(parentId) !== '';

                    if (hasParentCourse) {
                        const parentCourse = courses.find(c => String(c.id).toLowerCase() === String(parentId).toLowerCase());
                        const catId = parentCourse ? (parentCourse.category || 'skill') : (course.category || 'skill');
                        const cat = categories.find(c => String(c.id).toLowerCase() === String(catId).toLowerCase());
                        ...
                        setTimeout(() => {
                            const col2Items = megaMenu.querySelectorAll('.sidebar-column:nth-child(2) .sidebar-mega-item');
                            const parentName = parentCourse ? parentCourse.title : '';
                            col2Items.forEach(item => {
                                if (item.textContent.trim() === parentName) item.click();
                            });
                        }, 150);
                    } else {
                        const catId = course.category || 'skill';
                        const cat = categories.find(c => String(c.id).toLowerCase() === String(catId).toLowerCase());
                        const catName = cat ? cat.name : catId;
                        ...
                        const hasChildren = courses.some(c =>
                            String(c.parent_course_id).toLowerCase() === String(provider).toLowerCase() &&
                            c.is_deleted !== 1 && c.is_deleted !== '1' && c.is_deleted !== true
                        );
                        ...
                        if (hasChildren) {
                            setTimeout(() => {
                                const col2Items = megaMenu.querySelectorAll('.sidebar-column:nth-child(2) .sidebar-mega-item');
                                col2Items.forEach(item => {
                                    if (item.textContent.trim() === course.title) item.click();
                                });
                            }, 150);
                        }
                    }
...
// --- Sidebar Hierarchy helper (3-Column Layout) ---
// --- Sidebar Hierarchy helper (Dynamic Depth) ---
function populateSidebarHierarchy(courses, categories) {
    const container = document.getElementById('sidebar-mega-menu-container');
    if (!container) return;
    container.innerHTML = '';

    // Robust filter for deleted/inactive content
    const isDeleted = (item) => {
        if (!item) return true;
        const deleted = item.is_deleted;
        return deleted === 1 || deleted === '1' || deleted === true || deleted === 'true';
    };
    const isActive = (item) => {
    ...
    const activeCategories = categories.filter(cat => isActive(cat) && !isDeleted(cat));
    const activeCourses = courses.filter(c => isActive(c) && !isDeleted(c) && c.title && c.title.toLowerCase() !== 'null');

    // Unify nodes for easy tree traversal
    const allNodes = [
        ...activeCategories.map(c => ({ id: String(c.id).toLowerCase(), title: c.name, type: 'category', parentId: null, order: c.display_order || 0 })),
        ...activeCourses.map(c => ({
            id: String(c.id).toLowerCase(),
            title: c.title,
            type: activeCourses.some(child => String(child.parent_course_id).toLowerCase() === String(c.id).toLowerCase() && !isDeleted(child)) ? 'subcategory' : 'course',
            // Robust parent mapping
            parentId: (c.parent_course_id && String(c.parent_course_id).toLowerCase() !== "null" && String(c.parent_course_id) !== "") ? String(c.parent_course_id).toLowerCase() : (c.category ? String(c.category).toLowerCase() : null),
            icon: c.icon_class,
    ...
    const renderColumn = (depth, items, parentTitle) => {
        ...
        // Update sidebar width based on active columns
        ...
        items.sort((a, b) => (a.order || 0) - (b.order || 0) || a.title.localeCompare(b.title)).forEach(item => {
            ...
            el.className = 'sidebar-mega-item';

            const content = document.createElement('div');
            content.className = 'sidebar-course-item';
            if (item.icon) {
                content.innerHTML = `<i class="${item.icon}"></i> <span>${item.title}</span>`;
            } else {
            ...
            if (item.type !== 'course') {
                el.innerHTML += '<i class="fa-solid fa-chevron-right chevron"></i>';
            }

            el.onclick = (e) => {
                if (e) e.stopPropagation();
                // Update active state in this column
                column.querySelectorAll('.sidebar-mega-item').forEach(i => i.classList.remove('active'));
                el.classList.add('active');
                ...
                if (item.type !== 'course') {
                    ...
                    // Close sidebar ONLY via switchProvider which is robust
    ...
    // Initial render - ONLY the first column
    renderColumn(0, allNodes.filter(n => n.parentId === null));
}

async function renderSidebarCourses(categoryId, allCourses, coursesContainer, extensionsContainer) {
    // Deprecated by dynamic depth logic
}

async function renderSidebarExtensions(courseId, allCourses, extensionsContainer) {
    // Deprecated by dynamic depth logic
}

// Expose helper for mobile search
function filterSidebarCategories(query) {
    ...
    if (!q) {
        // Reset: remove search results, show original hierarchy
        const searchResults = container.querySelector('.sidebar-search-results');
        if (searchResults) searchResults.remove();
        container.querySelectorAll('.sidebar-column').forEach(col => col.style.display = '');
        return;
    }

    // Hide all hierarchy columns
    container.querySelectorAll('.sidebar-column').forEach(col => col.style.display = 'none');

    // Create or reuse flat search results column
    let searchResults = container.querySelector('.sidebar-search-results');
    if (!searchResults) {
        searchResults = document.createElement('div');
        searchResults.className = 'sidebar-column sidebar-search-results';
        container.appendChild(searchResults);
    }
    searchResults.style.display = '';
    searchResults.innerHTML = '';

    // Search through window.courses for matches
    const courses = window.courses || [];
    // Build set of parent IDs that have active sub-courses (hide these from results)
    const parentsWithChildren = new Set();
    courses.forEach(c => {
        if (c && c.parent_course_id && String(c.parent_course_id).toLowerCase() !== 'null' && String(c.parent_course_id) !== '' &&
            c.is_deleted !== 1 && c.is_deleted !== '1' && c.is_deleted !== true &&
            c.is_active !== 0 && c.is_active !== '0' && c.is_active !== false) {
            parentsWithChildren.add(String(c.parent_course_id).toLowerCase());
        }
    });
    const matches = courses.filter(c => {
        if (!c || !c.title || c.title.toLowerCase() === 'null') return false;
        if (c.is_deleted === 1 || c.is_deleted === '1' || c.is_deleted === true) return false;
        if (c.is_active === 0 || c.is_active === '0' || c.is_active === false) return false;
        // Hide parent courses that have sub-courses
        if (parentsWithChildren.has(String(c.id).toLowerCase())) return false;
        return c.title.toLowerCase().includes(q);
    });

    if (matches.length === 0) {
        searchResults.innerHTML = '<div style="padding: 16px; color: rgba(255,255,255,0.5); font-size: 13px;">No results found</div>';
        return;
    }

    matches.sort((a, b) => a.title.localeCompare(b.title)).forEach(course => {
        const el = document.createElement('div');
        el.className = 'sidebar-mega-item';
        const content = document.createElement('div');
        content.className = 'sidebar-course-item';
        content.innerHTML = (course.icon_class ? '<i class="' + course.icon_class + '"></i> ' : '') + '<span>' + course.title + '</span>';
        el.appendChild(content);
        el.onclick = () => {
            if (typeof switchProvider === 'function') switchProvider(course.id);
            if (typeof toggleSidebar === 'function') toggleSidebar();
        };
        searchResults.appendChild(el);
    });

    // Resize sidebar to single column
    const sidebarEl = document.getElementById('main-sidebar');
    if (sidebarEl) {
        sidebarEl.classList.remove('level-1', 'level-2', 'level-3');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log('Version: v2.1 - Sidebar Overhaul');
    ...
    try {
        ...
        // Global Filter: remove deleted or inactive items
        courses = courses.filter(c => c && c.id && String(c.is_active) !== '0' && String(c.is_active) !== 'false' && String(c.is_deleted) !== '1' && String(c.is_deleted) !== 'true' && c.title && c.title.toLowerCase() !== 'null');
        categories = categories.filter(cat => cat && cat.id && String(cat.is_active) !== '0' && String(cat.is_active) !== 'false' && String(cat.is_deleted) !== '1' && String(cat.is_deleted) !== 'true');
        window.courses = courses;

        // Populate Sidebar Hierarchy
        populateSidebarHierarchy(courses, categories);

        // Populate Mega Menu
        populateMegaMenu(courses, categories);
        ...
        if (recentcontainer) {
            ...
            if (!recentdata || recentdata.length === 0) {
                recentcontainer.innerHTML = `<div class="recent-topic-card" style="justify-content: center; color: #6b7280; cursor: default;"><span>Start exploring courses to see recent topics here</span></div>`;
            } else {
                for (let i = 0; i < recentdata.length; i++) {
                    const sim = recentdata[i];
                    try {
                        let coursedata = await window.learningapi.cachedapi.getcourse(sim.course_id);
                        // Skip deleted courses
                        if (!coursedata || coursedata.is_deleted == 1 || coursedata.is_deleted === true) continue;

                        let displayId = `${i + 1}`;
                        if (coursedata && coursedata.sections) {
                            let sIdx = 0;
                            outer: for (const sec of coursedata.sections) {
                                sIdx++;
                                let tIdx = 0;
                                for (const s of (sec.simulations || sec.items || [])) {
                                    tIdx++;
                                    if (s.id === sim.id) {
                                        displayId = `${sec.display_order || sIdx}.${s.display_order || tIdx}`;
                                        break outer;
                                    }
                                }
                            }
                        ...
                        card.innerHTML = `<div class="recent-topic-left"><span class="recent-topic-badge">${displayId}</span><span class="recent-topic-title">${sim.title || 'Untitled'}</span></div><span class="recent-topic-chip"><i class="${coursedata?.icon_class || 'fa-solid fa-book'}"></i> ${coursedata?.title || 'Course'}</span>`;
...
// --- Content Dropdown (Alphabetical List) Toggle ---
function toggleContentDropdown() {
    const panel = document.getElementById('content-dropdown-panel');
    if (panel) {
        panel.classList.toggle('show');
        // CLEAN FIX: Removed JS positioning using getBoundingClientRect.
        // Standard CSS (top: 100%, left: 0) inside relative parent works perfectly.
    }
}
window.toggleContentDropdown = toggleContentDropdown;
...
// Add event listener for content dropdown trigger + click-outside close
document.addEventListener('DOMContentLoaded', () => {
    const trigger = document.getElementById('content-nav-trigger');
    if (trigger) {
        trigger.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
    ...
    document.addEventListener('click', (e) => {
        ...
        if (panel && panel.classList.contains('show')) {
            // If click is outside both the panel and the trigger, close it
    ...
    // Close content dropdown on Escape key
...
// Course content sidebar search - filter sections and items
function filterchapters(inputEl, containerid) {
    const container = document.getElementById(containerid);
    if (!container) return;
    const q = inputEl.value.toLowerCase().trim();

    // Get all section headers and item containers
    const headers = container.querySelectorAll('.chapter-section-header');
    const allItems = container.querySelectorAll('.chapter-item');

    if (!q) {
        // Reset: show everything, restore collapse state
        headers.forEach(h => h.style.display = '');
        allItems.forEach(item => item.style.display = '');
        container.querySelectorAll('.chapter-section-items').forEach(el => {
            el.style.display = '';
            // Don't remove collapsed class - preserve user's collapse state
        });
        return;
    }

    // First: show ALL items, then hide non-matches
    allItems.forEach(item => {
        const text = item.textContent.toLowerCase();
        item.style.display = text.includes(q) ? '' : 'none';
    });

    // For each section: force-expand if it has matches, hide if no matches
    headers.forEach(header => {
        const headerText = header.textContent.toLowerCase();
        const headerMatches = headerText.includes(q);

        // Find the items container (next sibling with class chapter-section-items)
        let itemsContainer = header.nextElementSibling;
        while (itemsContainer && !itemsContainer.classList.contains('chapter-section-items')) {
            itemsContainer = itemsContainer.nextElementSibling;
        }

        if (itemsContainer) {
            // FORCE EXPAND: remove collapsed class and show
            itemsContainer.classList.remove('collapsed');
            itemsContainer.style.display = '';
            header.classList.remove('collapsed');

            const visibleItems = itemsContainer.querySelectorAll('.chapter-item:not([style*="display: none"])');

            if (headerMatches) {
                // Header matches: show all items in this section
                header.style.display = '';
                itemsContainer.querySelectorAll('.chapter-item').forEach(item => item.style.display = '');
            } else if (visibleItems.length > 0) {
                // Has matching items: show header + matching items
                header.style.display = '';
            } else {
                // No matches at all: hide entire section
                header.style.display = 'none';
                itemsContainer.style.display = 'none';
            }
        } else {
            header.style.display = headerMatches ? '' : 'none';
        }
    });
}
window.filterchapters = filterchapters;

// --- Content Dropdown (Alphabetical List) ---
function populateMegaMenu(courses, categories) {
    ...
    const activeCategoriesSet = new Set(categories.filter(cat => isActive(cat) && !isDeleted(cat)).map(cat => String(cat.id).toLowerCase()));

    const activeCourses = courses.filter(c => {
        if (!isActive(c) || isDeleted(c) || !c.title || c.title.toLowerCase() === 'null') return false;

        // "Apart from sidebar contents" filter: 
        // Only show courses that are either top-level or belong to an active category/parent
        const parentId = (c.parent_course_id && String(c.parent_course_id).toLowerCase() !== "null" && String(c.parent_course_id) !== "")
            ? String(c.parent_course_id).toLowerCase()
            : (c.category ? String(c.category).toLowerCase() : null);

        if (!parentId) return true; // Top level course (might happen if no category/parent)

        // If it has a parent, that parent must be an active category or another active course
        if (activeCategoriesSet.has(parentId)) return true;

        const parentCourse = courses.find(pc => String(pc.id).toLowerCase() === parentId);
        if (parentCourse && isActive(parentCourse) && !isDeleted(parentCourse)) return true;

        // If it belongs to a deleted/inactive category, skip it
        return false;
    });

    // Hide parent courses that have active sub-courses (they're just containers)
    // e.g. hide "Python" when "Python Main" and "py test" exist under it
    const parentsWithChildren = new Set();
    activeCourses.forEach(c => {
        if (c.parent_course_id && String(c.parent_course_id).toLowerCase() !== 'null' && String(c.parent_course_id) !== '') {
            parentsWithChildren.add(String(c.parent_course_id).toLowerCase());
        }
    });
    const displayCourses = activeCourses.filter(c => {
        return !parentsWithChildren.has(String(c.id).toLowerCase());
    });
    ...
    displayCourses.forEach(course => {
        ...
        // Handle numbers or special chars
    ...
    // Determine split point for two columns based on total item count for balance
    let totalItems = 0;
    sortedLetters.forEach(l => totalItems += (groups[l].length + 1.5)); // +1.5 for letter heading space

    let currentTotal = 0;
    let splitIndex = sortedLetters.length;
    for (let i = 0; i < sortedLetters.length; i++) {
        currentTotal += (groups[sortedLetters[i]].length + 1.5);
        if (currentTotal >= totalItems / 2) {
            splitIndex = i + 1;
            break;
        }
    }
    ...
    const columnsData = [
        sortedLetters.slice(0, splitIndex),
        sortedLetters.slice(splitIndex)
    ];

    columnsData.forEach((colLetters, colIdx) => {
        if (colLetters.length === 0) return;

        const column = document.createElement('div');
        column.className = 'alphabetical-column';
        ...
        colLetters.forEach(letter => {
            ...
            letterDiv.className = 'alphabetical-letter';
            letterDiv.textContent = letter === '#' ? '0-9' : letter;
            section.appendChild(letterDiv);

            groups[letter].sort((a, b) => a.title.localeCompare(b.title)).forEach(course => {
                const item = document.createElement('div');
                item.className = 'alphabetical-item';
                item.textContent = course.title;
                item.onclick = (e) => {
                    e.stopPropagation();
                    if (typeof switchProvider === 'function') switchProvider(course.id);
                    if (typeof closeContentDropdown === 'function') closeContentDropdown();
                };
        ...
        if (colIdx === 0 && columnsData[1].length > 0) {
            const divider = document.createElement('div');
            divider.className = 'alphabetical-divider';
    ...
    if (displayCourses.length === 0) {
        container.innerHTML = '<div style="padding: 20px; color: #666; text-align: center; width: 100%;">No content available</div>';
    }
}
window.populateMegaMenu = populateMegaMenu;
...
async function refreshCarousel() {
    ...
    try {
        ...
        if (cards.length > 0) {
            let html = '';
            const activeCards = cards.filter(card => card.is_active != 0 && card.is_active != false);
            ...
            if (activeCards.length > 0) {
                ...
                activeCards.forEach(card => {
                    ...
                    // Build onclick based on target type
                    let onclickAction;
                    if (targetType === 'course_page') {
                        const safeTitle = (card.title || '').replace(/'/g, "\\'");
                        const safeIcon = (icon || '').replace(/'/g, "\\'");
                        onclickAction = `switchProvider('${card.id}', null, { type: 'course_page', card_id: '${card.id}', card_title: '${safeTitle}', card_icon: '${safeIcon}', card_color: '${color}' })`;
                    } else {
                        onclickAction = `switchProvider('${targetId}', null, { type: '${targetType}', section_id: '${card.section_id || ''}' })`;
                    }

                    html += `
                            <div class="subject-card ${extraClass}" style="${cardStyle}" onclick="${onclickAction}">
                                <div>
                                    ...
                                    <button class="subject-start-btn" onclick="event.stopPropagation(); ${onclickAction};">
                ...
                // Fallback to mock data if no cards returned from API
                console.warn('No active carousel cards found, falling back to mock data');
                const mockCards = [
                    { id: 1, title: 'AWS Certified', description: 'Explore our interactive AWS labs and simulations.', icon_class: 'fa-brands fa-aws', color_hex: '#FF9900', target_type: 'course', target_id: 'aws', is_active: 1, display_order: 1 },
                    { id: 2, title: 'DevOps Mastery', description: 'Master CI/CD pipelines and infrastructure as code.', icon_class: 'fas fa-infinity', color_hex: '#A855F7', target_type: 'course', target_id: 'docker', is_active: 1, display_order: 2 },
                    { id: 3, title: 'Linux Admin', description: 'Build practical Linux command line skills.', icon_class: 'fa-brands fa-linux', color_hex: '#FCC624', target_type: 'course', target_id: 'linux', is_active: 1, display_order: 3 }
                ...
                mockCards.forEach(card => {
                    mockHtml += `
                            <div class="subject-card" style="--accent-color: ${card.color_hex};" onclick="switchProvider('${card.target_id}')">
                                <div>
                                    <div class="subject-icon"><i class="${card.icon_class}"></i></div>
                                    <div class="subject-title">${card.title}</div>
                                    <div class="subject-desc">${card.description}</div>
                                    <button class="subject-start-btn">Start Learning</button>
                                </div>
                            </div>
                        `;
                });
            ...
            carousels = [
                {
                    id: 2,
                    name: 'Carousel 2',
                    header: 'Explore. More Skills',
                    is_active: 1,
                    cards: [
                        { id: 101, title: 'Pro Design', description: 'Professional level layouts for modern apps.', color_hex: '#3b82f6', target_type: 'course', target_id: 'frontend', is_active: 1, content_type: 'image', image_url: 'https://raw.githubusercontent.com/sabs-27/images/main/card1.jpg', width: '400px', height_px: '500px' },
                        { id: 102, title: 'Cloud Native', description: 'Learn Kubernetes and Docker architecture.', color_hex: '#0ea5e9', target_type: 'course', target_id: 'kubernetes', is_active: 1, content_type: 'image', image_url: 'https://raw.githubusercontent.com/sabs-27/images/main/card2.jpg', width: '400px', height_px: '500px' }
                    ]
                }
            ];
        } else {
...
window.refreshDynamicCarousels = refreshDynamicCarousels;

// --- Carousel Course Page ---
// Opens a dedicated full-page view showing courses configured for a carousel card
async function openCarouselCoursePage(cardId, cardTitle, cardIcon, cardColor) {
    // Hide all course views and info pages
    document.querySelectorAll('.view-section').forEach(el => el.classList.add('hidden'));
    document.querySelectorAll('.info-page').forEach(el => el.classList.add('hidden'));
    document.body.classList.remove('is-course-view', 'is-topic-view', 'sidebar-open');
    ...
    // Hide home content (hero, carousels etc)
    const homeContent = viewHome ? viewHome.querySelectorAll('.hero-section, .home-lower-section') : [];
    homeContent.forEach(el => el.style.display = 'none');

    // Remove any existing course page
    let coursePage = document.getElementById('carousel-course-page');
    if (coursePage) coursePage.remove();

    // Create the course page container
    coursePage = document.createElement('div');
    coursePage.id = 'carousel-course-page';
    coursePage.className = 'carousel-course-page';
    coursePage.innerHTML = `
        <div class="ccp-header" style="--ccp-color: ${cardColor};">
            <button class="ccp-back-btn" onclick="closeCarouselCoursePage()">
                <i class="fa-solid fa-arrow-left"></i> Back to Home
            </button>
            <div class="ccp-header-content">
                <div class="ccp-header-icon"><i class="${cardIcon}"></i></div>
                <h1 class="ccp-header-title">${cardTitle}</h1>
                <p class="ccp-header-subtitle">Explore courses and start learning</p>
            </div>
        </div>
        <div class="ccp-body">
    ...
    `;
    viewHome.appendChild(coursePage);
    ...
    history.pushState({ level: 'course_page', cardId }, '', window.location.href);
    window.currentNavLevel = 'course_page';

    // Fetch course links from API
    try {
        const res = await fetch(`${API_CONFIG.baseUrl}/api/carousel-card/${cardId}/courses?t=${Date.now()}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        const courseLinks = data.course_links || [];

        const bodyEl = coursePage.querySelector('.ccp-body');
        ...
        if (courseLinks.length === 0) {
            bodyEl.innerHTML = `
                <div class="ccp-empty">
                    ...
                    <button class="ccp-back-link" onclick="closeCarouselCoursePage()">← Back to Home</button>
                </div>
            `;
            return;
        }

        let cardsHtml = '<div class="ccp-grid">';
        courseLinks.forEach((link, index) => {
            const courseId = link.course_id || '';
            const courseTitle = link.title || 'Untitled';
            const courseDesc = link.description || '';
            const courseIcon = link.icon || 'fa-solid fa-book';

            cardsHtml += `
                <div class="ccp-course-card" onclick="openCourseAutoFirst('${courseId}');" style="animation-delay: ${index * 0.06}s">
                    <div class="ccp-course-icon" style="color: ${cardColor};">
                        <i class="${courseIcon}"></i>
                    </div>
                    <div class="ccp-course-info">
                        <div class="ccp-course-title">${courseTitle}</div>
                        ${courseDesc ? `<div class="ccp-course-desc">${courseDesc}</div>` : ''}
                    </div>
                    <div class="ccp-course-arrow">
                        <i class="fa-solid fa-chevron-right"></i>
                    </div>
                </div>
            `;
        ...
        console.error('Failed to load carousel course page:', e);
        const bodyEl = coursePage.querySelector('.ccp-body');
        bodyEl.innerHTML = `
            <div class="ccp-empty">
...
window.openCarouselCoursePage = openCarouselCoursePage;

function closeCarouselCoursePage() {
    const coursePage = document.getElementById('carousel-course-page');
    if (coursePage) coursePage.remove();
...
window.closeCarouselCoursePage = closeCarouselCoursePage;

// Opens a course from the course page and auto-selects the first topic
function openCourseAutoFirst(courseId) {
    closeCarouselCoursePage();

    // Resolve courseId: if it doesn't match a known course ID, try matching by title
    // This handles the case where course_links stored a display name instead of a real ID
    const courses = window.courses || [];
    let resolvedId = courseId;

    // Check if courseId directly matches a course
    const directMatch = courses.find(c => c.id === courseId);
    if (!directMatch) {
        // Try case-insensitive title match
        const titleMatch = courses.find(c =>
            c.title && c.title.toLowerCase().trim() === courseId.toLowerCase().trim()
        );
        if (titleMatch) {
            console.log(`[openCourseAutoFirst] Resolved "${courseId}" → "${titleMatch.id}" via title match`);
            resolvedId = titleMatch.id;
        } else {
            ...
            const partialMatch = courses.find(c =>
                c.title && (
                    ...
                    courseId.toLowerCase().includes(c.title.toLowerCase())
                )
            );
            if (partialMatch) {
                console.log(`[openCourseAutoFirst] Resolved "${courseId}" → "${partialMatch.id}" via partial match`);
                resolvedId = partialMatch.id;
            }
        }
    }

    // Navigate to the course — do NOT rely on callback
    switchProvider(resolvedId);

    // Poll until sidebar renders, then force-expand first section and click first topic
    let pollCount = 0;
    const poller = setInterval(() => {
        pollCount++;
        if (pollCount > 35) { clearInterval(poller); return; } // give up after ~10s

        // Look for chapter section headers in the sidebar
        const headers = document.querySelectorAll('.chapter-section-header');
        if (headers.length === 0) return; // sidebar not rendered yet, keep polling

        // Found headers! Force-expand the first section
        const firstHeader = headers[0];
        const sectionId = firstHeader.id ? firstHeader.id.replace('header-', '') : null;

        // Remove collapsed from header
        firstHeader.classList.remove('collapsed');

        // Remove collapsed from items container
        if (sectionId) {
            const itemsContainer = document.getElementById('items-' + sectionId);
            if (itemsContainer) {
                itemsContainer.classList.remove('collapsed');

                // Find and click the first chapter-item
                const firstItem = itemsContainer.querySelector('.chapter-item');
                if (firstItem) {
        ...
        const nextEl = firstHeader.nextElementSibling;
        if (nextEl && nextEl.classList.contains('chapter-section-items')) {
            nextEl.classList.remove('collapsed');
            const firstItem = nextEl.querySelector('.chapter-item');
            if (firstItem) {
                clearInterval(poller);
                firstItem.click();
                return;
...
// Track when user opens a course (update nav level tracking)
document.addEventListener('click', function (e) {
    const courseItem = e.target.closest('.sidebar-item[onclick*="opencourse"], .sidebar-item[onclick*="switchProvider"], .sidebar-mega-item[onclick*="switchProvider"]');
    if (courseItem) {
        const onclick = courseItem.getAttribute('onclick') || '';
        const match = onclick.match(/switchProvider\(['"]([^'"]+)['"]\)/);
        if (match) {
            window.currentCourseId = match[1];
            window.currentNavLevel = 'course';
        }
    ...
    const topicItem = e.target.closest('.chapter-item, .sidebar-extension-link');
    if (topicItem) {
        window.currentNavLevel = 'topic';
    }
});
