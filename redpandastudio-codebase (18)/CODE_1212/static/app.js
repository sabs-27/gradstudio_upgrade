        ...
        async function renderMegaMenuCourses(categoryId, allCourses, coursesContainer, topicsContainer) {
            ...
            const catCourses = allCourses
                .filter(c => (c.category || 'skill') === categoryId)
            ...
            catCourses.forEach(course => {
                ...
                item.onmouseenter = async () => {
                    ...
                    renderMegaMenuTopics(course.id, course.title, topicsContainer);
                };
        ...
        async function renderMegaMenuTopics(courseId, courseTitle, topicsContainer) {
            topicsContainer.innerHTML = '<div class="mega-menu-empty"><i class="fa-solid fa-spinner fa-spin"></i> Loading...</div>';
            
            try {
                const courseData = await window.learningapi.cachedapi.getcourse(courseId);
                if (!courseData || !courseData.sections || courseData.sections.length === 0) {
                    topicsContainer.innerHTML = '<div class="mega-menu-empty">No topics available</div>';
                    return;
                }
                ...
                if (!html) {
                    topicsContainer.innerHTML = '<div class="mega-menu-empty">No topics found</div>';
                } else {
                    topicsContainer.innerHTML = html;
                }
            } catch (err) {
                console.error('Mega menu topics load failed:', err);
                topicsContainer.innerHTML = '<div class="mega-menu-empty">Error loading topics</div>';
            }
        }
            ...
            try {
                ...
                categories.forEach((cat, catindex) => {
                    ...
                    if (catcourses.length === 0) {
                        ...
                        catcourses.forEach(c => ul.appendChild(createcourseitem(c)));
                    }
