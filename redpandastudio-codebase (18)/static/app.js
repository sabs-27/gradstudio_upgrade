        ...
        function populateSidebarHierarchy(courses, categories) {
            ...
            const allNodes = [
                ...activeCategories.map(c => ({ id: String(c.id).toLowerCase(), title: c.name, type: 'category', parentId: null })),
                ...activeCourses.map(c => {
                    const hasChildren = activeCourses.some(child => String(child.parent_course_id).toLowerCase() === String(c.id).toLowerCase() && !isDeleted(child));
                    // Respect DB type if available, otherwise infer
                    let type = c.type || (hasChildren ? 'subcategory' : 'course');
                    // Force subcategory type if it has children to avoid dead-ends in UI
                    if (hasChildren) type = 'subcategory';
        ...
        function filterSidebarCategories(query) {
            ...
            if (!container) return;
            const items = container.querySelectorAll('.sidebar-mega-item');
        ...
        // Add event listener for content dropdown trigger
        ...
        function populateMegaMenu(courses, categories) {
            ...
            const activeCourses = courses.filter(c => {
                // Strict filter: must be active, not deleted, and NOT a subcourse (only top-level courses in alphabetical list)
                // Actually, the user says "match sidebar contents", so we should show all active courses but maybe grouped?
                // The image shows a flat alphabetical list of topics.
                if (!isActive(c) || isDeleted(c) || !c.title || c.title.toLowerCase() === 'null') return false;
                
                // If the user wants it to match sidebar, we should probably only show items that are visible.
                // Orphaned items (no category and no parent) are generally not visible in the sidebar.
                ...
                if (!parentId) return true; // Top level
