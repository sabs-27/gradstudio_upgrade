        ...
        async function switchprovider(provider, callback, opts = {}) {
            ...
            // Map provider IDs to actual course IDs from database
            // Use mock data IDs when USE_MOCK_DATA is true, otherwise use production IDs
            const courseIdMap = USE_MOCK_DATA ? {
                'python': 'python',   // Mock data uses 'python'
                'gcp': 'python',      // Also support 'gcp' for Python
                'sql': 'sql',         // Mock data uses 'sql' (if exists, otherwise will use provider)
                'docker': 'docker',   // Mock data uses 'docker'
                'aws': 'aws',
                'linux': 'linux',
                'backend': 'backend',
                'devops': 'backend',  // Map devops to backend in mock
                'azure': 'azure',     // Mock data has azure
                'frontend': 'frontend',
                'ai': 'ai'
            } : {
                'python': 'python1',  // Production: Python card maps to 'python1' course ID
                'gcp': 'python1',     // Also support 'gcp' for Python
                'sql': 'sql1',        // Production: SQL card maps to 'sql1' course ID
                'docker': 'ml2',      // Production: Machine Learning card maps to 'ml2' course ID
                'aws': 'aws',
                'linux': 'linux',
                'backend': 'backend1',
                'devops': 'ai1'
            ...
            const actualProvider = courseIdMap[provider] || provider;
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            async function refreshCarousel() {
                ...
                try {
                    const res = await fetch(`${API_CONFIG.baseUrl}/api/carousel?t=${Date.now()}`);
                    ...
                    if (cards.length > 0) {
                        ...
                        cards.forEach(card => {
                            // Map icon class if needed
                            const icon = card.icon_class || 'fa-solid fa-star';
                            
                            // Map card classes for gradients (fallbacks)
                            let cardClass = 'card-default';
                            const titleLower = card.title.toLowerCase();
                            if (titleLower.includes('aws')) cardClass = 'card-aws';
                            else if (titleLower.includes('ai')) cardClass = 'card-devops';
                            else if (titleLower.includes('machine') || titleLower.includes('learning')) cardClass = 'card-docker';
                            else if (titleLower.includes('python')) cardClass = 'card-gcp';
                            else if (titleLower.includes('linux')) cardClass = 'card-linux';
                            else if (titleLower.includes('sql')) cardClass = 'card-sql';
                            else if (titleLower.includes('backend')) cardClass = 'card-backend';
                            else if (titleLower.includes('java')) cardClass = 'card-java';

                            html += `
                                <div class="subject-card ${cardClass}" onclick="switchProvider('${card.target_id || ''}')">
                                    <div>
                                        ...
                                        <button class="subject-start-btn" onclick="event.stopPropagation(); switchProvider('${card.target_id || ''}');">
            ...
            window.addEventListener('storage', (e) => {
                if (e.key === 'lp_admin_refresh') refreshCarousel();
            });
