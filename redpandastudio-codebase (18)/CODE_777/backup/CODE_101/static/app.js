        ...
        const MOCK_DATA = {
            ...
            courses: [
                { id: 'azure', title: 'Azure', icon_class: 'fa-brands fa-microsoft', category: 'skill', is_locked: true },
                { id: 'linux', title: 'Linux', icon_class: 'fa-brands fa-linux', category: 'skill', is_locked: true },
                { id: 'aws', title: 'Amazon AWS', icon_class: 'fa-brands fa-aws', category: 'skill', is_locked: true },
                { id: 'python', title: 'Python', icon_class: 'fa-brands fa-python', category: 'skill', is_locked: true },
                { id: 'docker', title: 'Docker', icon_class: 'fa-brands fa-docker', category: 'skill', is_locked: true },
                { id: 'kubernetes', title: 'Kubernetes', icon_class: 'fa-solid fa-cube', category: 'skill', is_locked: true },
                { id: 'terraform', title: 'Terraform', icon_class: 'fa-solid fa-mountain', category: 'skill', is_locked: true },
                { id: 'gcp', title: 'Google Cloud', icon_class: 'fa-brands fa-google', category: 'skill', is_locked: true },
                { id: 'frontend', title: 'Front End', icon_class: 'fa-brands fa-html5', category: 'role', is_locked: true },
                { id: 'ai', title: 'AI', icon_class: 'fa-solid fa-brain', category: 'role', is_locked: true },
                { id: 'backend', title: 'Backend', icon_class: 'fa-solid fa-server', category: 'role', is_locked: true }
            ],
            courseData: {
                azure: {
                    id: 'azure',
        ...
        function restorecollapsedstates() {
            ...
            const collapsedcategories = JSON.parse(localStorage.getItem('collapsedcategories') || '{}');
            Object.keys(collapsedcategories).forEach(catid => {
                if (collapsedcategories[catid]) {
                    const header = document.getElementById(`sidebar-header-${catid}`);
        ...
        async function loadandrendercourse(courseid, containerid, title, iconclass) {
            console.log('[loadandrendercourse] Called with courseid:', courseid, 'containerid:', containerid, 'title:', title);
            ...
            try {
                ...
                rendercourseview(coursedata, containerid, title, iconclass, courseid);
        ...
        function rendercourseview(coursedata, containerid, title, iconclass, courseid) {
            console.log('rendercourseview called for:', containerid, 'with data:', coursedata);  // debug
            const container = document.getElementById(containerid);
            ...
            if (screen && !showcourselock) {
                ...
                if (isMobile) {
                    ...
                    // Only show landing if no topic is being restored
        ...
        async function switchprovider(provider, callback, opts = {}) {
            ...
            if (view && view.querySelector('.chapter-sidebar')) {
                ...
                // On mobile, ensure landing page is cleared when switching courses
                const isMobile = window.matchMedia('(max-width: 900px)').matches;
                if (isMobile) {
                    ...
                    if (viewerScreen) {
                        ...
                        // Only clear if there's landing content, not if there's already simulation content
                        if (viewerScreen.querySelector('.course-landing') || viewerScreen.classList.contains('has-landing')) {
                ...
                const footerspan = document.querySelector('.footer span:first-child') || document.querySelector('.main-footer span:first-child');
                if (footerspan) {
                    const count = window.courseitemcounts?.[actualProvider] || 0;
                    footerspan.textContent = `Course contents: ${count}`;
                }
                if (callback) callback();
                return Promise.resolve();
            }
            ...
            return new Promise(async (resolve) => {
                ...
                try {
                    ...
                    console.log('[switchprovider] Loading course:', course.id, 'with title:', course.title);
                    ...
                    const footerspan = document.querySelector('.footer span:first-child') || document.querySelector('.main-footer span:first-child');
                    if (footerspan) {
                        footerspan.textContent = `Course contents: ${itemcount}`;
                    }
        ...
        function showPage(pageId) {
            ...
            document.querySelectorAll('.header-nav-link').forEach(el => el.classList.remove('active'));
        ...
        // --- Content dropdown helper ---
        function populateContentDropdownInline(courses, categories) {
            const grid = document.getElementById('content-dropdown-grid');
            if (!grid) return;
            grid.innerHTML = '';
            if (!courses || courses.length === 0 || !categories || categories.length === 0) return;

            // Group by category, then by first letter
            categories.forEach(cat => {
                const catCourses = courses
                    .filter(c => c && c.title && c.id && (c.category || 'skill') === cat.id)
                    .sort((a, b) => a.title.localeCompare(b.title));
                if (catCourses.length === 0) return;

                const col = document.createElement('div');
                col.className = 'content-dropdown-col';
                const header = document.createElement('div');
                header.className = 'content-dropdown-col-header';
                ...
                catCourses.forEach(course => {
                    ...
                    if (firstLetter !== currentLetter) {
                        ...
                        letterDiv.className = 'content-dropdown-letter';
                    ...
                    link.onclick = (e) => { e.preventDefault(); switchProvider(course.id); };
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                // helper function to create course sidebar item
                ...
                // if no categories, maybe fallback or show empty
                if (categories.length === 0 && courses.length > 0) {
                    // only happens if db empty or fetch failed.
                    // we can synthesize 'skill based' if absolutely needed, but let's trust api.
                }

                categories.forEach((cat, catindex) => {
                    // filter courses for this category
                    const catcourses = courses.filter(c => (c.category || 'skill') === cat.id);

                    // only show category if it has courses? or show empty? 
                    // user said: "category must appear in website sidebar (empty state allowed)"?
                    // "category must appear in website sidebar (empty state allowed)" -> yes.
                    ...
                    if (catcourses.length === 0) {
                        ...
                        const visibleCourses = catcourses.slice(0, 4);
                        const hiddenCourses = catcourses.slice(4);

                        visibleCourses.forEach(c => ul.appendChild(createcourseitem(c)));
                        hiddenCourses.forEach(c => {
                            const item = createcourseitem(c);
                        ...
                        if (hiddenCourses.length > 0) {
                            ...
                            expandBtn.id = `expand-${cat.id}`;
                            expandBtn.setAttribute('data-expand-label', `Expand all (${hiddenCourses.length} more)`);
                            expandBtn.textContent = `Expand all (${hiddenCourses.length} more)`;
                            expandBtn.onclick = (e) => {
                                e.stopPropagation();
                                toggleexpandcourses(cat.id);
                            };
                            box.appendChild(ul);
            ...
            const footerSpan = document.querySelector('.footer span:first-child') || document.querySelector('.main-footer span:first-child');
            if (footerSpan) {
                footerSpan.textContent = 'Welcome home';
            }
            ...
            async function refreshCarousel() {
                ...
                try {
                    ...
                    if (cards.length > 0) {
                        let html = '';
                        cards.sort((a, b) => (a.display_order || 0) - (b.display_order || 0));
                        cards.forEach(card => {
                            // Skip inactive cards
                            if (card.is_active === 0 || card.is_active === false) return;

                            const icon = card.icon_class || 'fa-solid fa-star';
                            const color = card.color_hex || '#3b82f6';
                            
                            // Set the accent color variable for the dark glossy styling
                            let cardStyle = `--accent-color: ${color};`;
                            
                            const targetType = card.target_type || 'course';
                            const targetId = card.target_id || '';
                            
                            // Map icon characters for the ::after effect
                            let extraClass = '';
                        ...
                        html += `
                            <div class="subject-card ${extraClass}" style="${cardStyle}" onclick="switchProvider('${targetId}', null, { type: '${targetType}' })">
                                <div>
                                    ...
                                    <button class="subject-start-btn" onclick="event.stopPropagation(); switchProvider('${targetId}', null, { type: '${targetType}' });">
                ...
                console.error('Failed to load dynamic carousel:', e);
            }
        }

            async function refreshSecondaryCarousel() {
                const container = document.getElementById('secondary-carousel');
                const headerElem = document.getElementById('secondary-carousel-header');
                if (!container) return;
    
                try {
                    // Use simple fetch to avoid preflight, add cache-busting t param
                    const res = await fetch(`${API_CONFIG.baseUrl}/api/carousel?type=2&t=${Date.now()}`);
                    if (!res.ok) throw new Error(`HTTP ${res.status}`);
                    
                    const data = await res.json();
                    const cards = data.cards || [];
                    
                    // Update header from metadata if available
                    if (headerElem) {
                        ...
                        if (headerText) {
                            ...
                            // Robust splitting: split only at the first period if it exists
                            ...
                            if (dotIndex !== -1) {
                                ...
                                headerElem.innerHTML = `<span class="header-bold">${firstPart}.</span> <span class="header-gray">${secondPart}</span>`;
                            } else {
                                headerElem.innerHTML = `<span class="header-bold">${headerText}</span>`;
                            }
                        } else {
                    ...
                    if (cards.length > 0) {
                        ...
                        // Filter active cards first to find the true first card for height
                        const activeCards = cards.filter(c => c.is_active !== 0 && c.is_active !== false && c.is_active !== '0');
                        
                        if (activeCards.length > 0) {
                            // UNIFORM HEIGHT ENFORCEMENT
                            ...
                            if (contentType === 'image' && card.image_url) {
                                ...
                                // HEAL BROKEN HTML: Unescape common entities if present to ensure proper rendering
                            ...
                            const clickAction = (targetType === 'none' || !targetId || targetId === 'none') 
                                ? '' 
                                : `onclick="switchProvider('${targetId}', null, { type: '${targetType}' })"`;
                        ...
                        container.innerHTML = html;
                            ...
                            container.innerHTML = '<div style="padding:40px; color:#666; text-align:center; width:100%;">No interactive concepts available yet.</div>';
                        }
                    } else {
                        container.innerHTML = '<div style="padding:40px; color:#666; text-align:center; width:100%;">No interactive concepts available yet.</div>';
                    }
                } catch (e) {
                    console.error('Failed to load secondary carousel:', e);
                    container.innerHTML = '<div style="padding:40px; color:#ef4444; text-align:center; width:100%;">Failed to load content.</div>';
                }
            }

        function scrollSecondaryCarousel(direction) {
            const container = document.getElementById('secondary-carousel');
            if (!container) return;
            const scrollAmount = window.innerWidth > 900 ? 420 : 320;
            container.scrollBy({ left: direction * scrollAmount, behavior: 'smooth' });
        }
        window.scrollSecondaryCarousel = scrollSecondaryCarousel;
