        ...
        function populateContentDropdownInline(courses, categories) {
            const categoriesCol = document.getElementById('mega-menu-categories');
            const coursesCol = document.getElementById('mega-menu-courses');
            const topicsCol = document.getElementById('mega-menu-topics');
            
            if (!categoriesCol || !coursesCol || !topicsCol) return;
            
            categoriesCol.innerHTML = '';
            coursesCol.innerHTML = '<div class="mega-menu-empty">Select a category</div>';
            topicsCol.innerHTML = '<div class="mega-menu-empty">Select a course</div>';
            ...
            categories.forEach(cat => {
                ...
                item.innerHTML = `<span>${cat.name}</span> <i class="fa-solid fa-chevron-right chevron"></i>`;
                
                const triggerAction = () => {
                    // Update active state
                    categoriesCol.querySelectorAll('.mega-menu-item').forEach(el => el.classList.remove('active'));
                    item.classList.add('active');
                    
                    // Show courses for this category
                    renderMegaMenuCourses(cat.id, courses, coursesCol, topicsCol);
                };

                item.onmouseenter = triggerAction;
                item.onclick = (e) => {
                ...
                categoriesCol.appendChild(item);
            });

            // Initial state: select first category if available
            if (categories.length > 0) {
                const firstCat = categoriesCol.firstChild;
                if (firstCat && firstCat.onmouseenter) firstCat.onmouseenter();
            }
        }

        async function renderMegaMenuCourses(categoryId, allCourses, coursesContainer, topicsContainer) {
            coursesContainer.innerHTML = '';
            topicsContainer.innerHTML = '<div class="mega-menu-empty">Select a course</div>';
            
            // Only show TOP-LEVEL courses for this category (no parent_course_id)
            const catCourses = allCourses
                .filter(c => (c.category || 'skill') === categoryId && !c.parent_course_id)
                .sort((a, b) => a.title.localeCompare(b.title));
            
            if (catCourses.length === 0) {
                coursesContainer.innerHTML = '<div class="mega-menu-empty">No courses found</div>';
                return;
            }
            ...
            catCourses.forEach(course => {
                ...
                const isAdmin = localStorage.getItem('lp_auth_token') || localStorage.getItem('lp_is_logged_in') === 'true';
                
                item.innerHTML = `
                    <div class="mega-menu-course-item">
                        <i class="${course.icon_class || 'fa-solid fa-book'}"></i>
                        <span>${course.title}</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ${isAdmin ? `<button class="mega-menu-admin-plus" onclick="event.stopPropagation(); window.location.href='/admin-edit.html?course=${course.id}&action=edit'" title="Edit Course" style="background:rgba(249, 115, 22, 0.2); border:1px solid #f97316; color:#f97316; border-radius:4px; width:22px; height:22px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:10px;"><i class="fa-solid fa-pencil"></i></button>` : ''}
                        <i class="fa-solid fa-chevron-right chevron"></i>
                    </div>
                `;
                
                const triggerCourseAction = async () => {
                    // Update active state
                    coursesContainer.querySelectorAll('.mega-menu-item').forEach(el => el.classList.remove('active'));
                    item.classList.add('active');
                    
                    // Show extensions (sub-courses + sections) for this course
                    renderMegaMenuExtensions(course.id, allCourses, topicsContainer);
                };

                item.onmouseenter = triggerCourseAction;
            ...
            // Initial state: select first course if available
            if (catCourses.length > 0) {
                const firstCourse = coursesContainer.firstChild;
                if (firstCourse && firstCourse.onmouseenter) firstCourse.onmouseenter();
            }
        }

        async function renderMegaMenuExtensions(courseId, allCourses, extensionsContainer) {
            extensionsContainer.innerHTML = '<div class="mega-menu-empty"><i class="fa-solid fa-spinner fa-spin"></i> Loading...</div>';
            
            try {
                // 1. Get Sub-courses
                const subCourses = allCourses
                    .filter(c => c.parent_course_id === courseId)
                    .sort((a, b) => (a.display_order || 0) - (b.display_order || 0));

                // 2. Get Sections & Topics (Topics)
                const courseData = await window.learningapi.cachedapi.getcourse(courseId);
                
                let html = '';

                // Render Sub-courses first (Udemy style "Sub-categories")
                if (subCourses.length > 0) {
                    html += `<div class="mega-menu-topic-group">`;
                    html += `<div class="mega-menu-topic-header" style="color:var(--accent); font-weight:700;">SUB-COURSES</div>`;
                    subCourses.forEach(sub => {
                        html += `<a href="#" class="mega-menu-topic-link" style="font-weight:600;" onclick="event.preventDefault(); switchProvider('${sub.id}'); closeContentDropdown();"><i class="${sub.icon_class || 'fas fa-book'}" style="margin-right:8px; font-size:10px; opacity:0.7;"></i> ${sub.title}</a>`;
                    });
                    html += `</div>`;
                }
                ...
                if (courseData && courseData.sections && courseData.sections.length > 0) {
                    courseData.sections.slice(0, 5).forEach(section => {
                        const items = section.simulations || section.items || [];
                        if (items.length === 0) return;

                        html += `<div class="mega-menu-topic-group">`;
                        html += `<div class="mega-menu-topic-header">${section.title}</div>`;
                        
                        items.slice(0, 6).forEach(topic => {
                            html += `<a href="#" class="mega-menu-topic-link" onclick="event.preventDefault(); window.openTopicById('${courseId}', '${topic.id}'); closeContentDropdown();">${topic.title || topic.name}</a>`;
                        });
                ...
                if (!html) {
                    extensionsContainer.innerHTML = '<div class="mega-menu-empty">No content available</div>';
                } else {
                    extensionsContainer.innerHTML = html;
                }
            } catch (err) {
                console.error('Mega menu extensions load failed:', err);
                extensionsContainer.innerHTML = '<div class="mega-menu-empty">Error loading content</div>';
            }
        }
