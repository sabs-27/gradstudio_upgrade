      ...
      try {
        ...
        if (path === '/api/auth/google/callback' && request.method === 'GET') {
          return await handleGoogleAuthCallback(request, env, corsHeaders);
            ...
            // MIGRATION: Add parent_course_id and type columns if missing + PERMANENT CLEANUP
            if (path === '/api/admin/migrate' && request.method === 'POST') {
              ...
              const run = async (sql) => {
                try {
                  ...
                  logs.push(`SUCCESS: ${sql.slice(0, 50)}...`);
                } catch (e) {
                  logs.push(`ERROR: ${sql.slice(0, 50)}... -> ${e.message}`);
                }
              };

              // 1. Column Migrations (Try each one individually)
              await run(`ALTER TABLE courses ADD COLUMN parent_course_id TEXT DEFAULT NULL`);
              await run(`ALTER TABLE courses ADD COLUMN type TEXT DEFAULT 'course'`);
              await run(`ALTER TABLE courses ADD COLUMN is_deleted BOOLEAN DEFAULT 0`);
              await run(`ALTER TABLE courses ADD COLUMN deleted_at TEXT DEFAULT NULL`);
              
              await run(`ALTER TABLE sections ADD COLUMN is_deleted INTEGER DEFAULT 0`);
              await run(`ALTER TABLE sections ADD COLUMN deleted_at INTEGER`);
              
              await run(`ALTER TABLE simulations ADD COLUMN is_deleted INTEGER DEFAULT 0`);
              await run(`ALTER TABLE simulations ADD COLUMN deleted_at INTEGER`);
              
              await run(`ALTER TABLE categories ADD COLUMN is_deleted INTEGER DEFAULT 0`);
              await run(`ALTER TABLE categories ADD COLUMN deleted_at INTEGER`);

              // 2. Data Fixes
              await run(`UPDATE courses SET type = 'subcategory' WHERE id IN (SELECT DISTINCT parent_course_id FROM courses WHERE parent_course_id IS NOT NULL AND parent_course_id != '')`);
              
              // 3. HARD CLEANUP - DELETE ALL ORPHANED/DELETED RECORDS (User Requested)
              // We delete in specific order to avoid foreign key conflicts
              await run(`DELETE FROM statistics WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);
              await run(`DELETE FROM comments WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);
              await run(`DELETE FROM simulation_likes WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);
              await run(`DELETE FROM simulation_dislikes WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);
              await run(`DELETE FROM simulation_reports WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);
              await run(`DELETE FROM bookmarks WHERE simulation_id IN (SELECT id FROM simulations WHERE is_deleted = 1)`);

              await run(`DELETE FROM simulations WHERE is_deleted = 1`);
              
              // Cleanup sections for deleted courses
              await run(`DELETE FROM simulations WHERE section_id IN (SELECT id FROM sections WHERE is_deleted = 1)`);
              await run(`DELETE FROM sections WHERE is_deleted = 1`);
              
              // Cleanup courses
              await run(`DELETE FROM sections WHERE course_id IN (SELECT id FROM courses WHERE is_deleted = 1)`);
              await run(`DELETE FROM simulations WHERE course_id IN (SELECT id FROM courses WHERE is_deleted = 1)`);
              await run(`DELETE FROM courses WHERE is_deleted = 1`);
              
              await run(`DELETE FROM categories WHERE is_deleted = 1`);

              // 4. Ensure Table Integrity
              await run(`CREATE TABLE IF NOT EXISTS carousel_cards (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT, icon_class TEXT, color_hex TEXT DEFAULT '#3b82f6', target_type TEXT, target_id TEXT, display_order INTEGER DEFAULT 0, is_active BOOLEAN DEFAULT 1, created_at TEXT DEFAULT (datetime('now')), content_type TEXT DEFAULT 'standard', image_url TEXT, iframe_url TEXT, content_html TEXT, width TEXT DEFAULT '300px', full_bleed INTEGER DEFAULT 0, carousel_id INTEGER DEFAULT 1, height_px TEXT DEFAULT 'auto')`);
              await run(`CREATE TABLE IF NOT EXISTS carousels (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, header TEXT, is_active BOOLEAN DEFAULT 1, display_order INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`);
              await run(`CREATE TABLE IF NOT EXISTS metadata (key TEXT PRIMARY KEY, value TEXT, updated_at TEXT DEFAULT (datetime('now')))`);

              await env.KV.delete('courses:all').catch(() => {});
              return jsonResponse({ success: true, message: 'CRITICAL REPAIR: Schema updated and all deleted content PERMANENTLY PURGED.', logs }, 200, corsHeaders);
            }
...
async function handleGetAllCourses(request, env, corsHeaders) {
  ...
  const whereClause = isAdminRequest ? 'c.is_deleted = 0' : 'c.is_active = 1 AND c.is_deleted = 0';

  try {
    let { results } = await env.DB.prepare(`
      SELECT 
        c.id, c.title, c.icon_class, c.description, c.color_theme, c.category, c.display_order, c.is_active, c.is_locked, c.type, c.parent_course_id, c.is_deleted,
    ...
    const allCourses = results.map(row => ({
      ...
      type: row.type || (row.parent_course_id ? 'course' : 'subcategory'),
      simulation_count: row.simulation_count || 0
    }));

    const responseData = { courses: allCourses, total: allCourses.length };
    ...
    console.error('Fetch courses error, trying fallback:', err.message);
    
    const hasType = !err.message.includes('type');
    const hasParent = !err.message.includes('parent_course_id');
    const hasDeleted = !err.message.includes('is_deleted');
    
    const fallbackWhere = isAdminRequest ? '1=1' : 'c.is_active = 1';
    
    let { results } = await env.DB.prepare(`
      SELECT 
        c.id, c.title, c.icon_class, c.category, c.display_order, c.is_active, c.is_locked,
        ${hasType ? 'c.type' : "'course' as type"},
        ${hasParent ? 'c.parent_course_id' : 'NULL as parent_course_id'},
        ${hasDeleted ? 'c.is_deleted' : '0 as is_deleted'},
        COUNT(sim.id) as simulation_count
      FROM courses c
    ...
    return jsonResponse({ courses: allCourses, total: allCourses.length }, 200, corsHeaders);
  }
}
...
async function handleGetCourse(courseId, request, env, corsHeaders) {
  ...
  try {
    ...
    // Robust fallback if columns missing
    console.error('Fetch course details error, trying fallback:', err.message);
    ...
    ({ results } = await env.DB.prepare(`
      SELECT 
        ...
        sim.display_order as sim_order,
        sim.upload_date,
        0 as views, 0 as likes, 0 as dislikes, 0 as reports
      FROM courses c
  ...
  if (!results || results.length === 0) {
    const course = await env.DB.prepare('SELECT id, title, icon_class, description, is_locked FROM courses WHERE id = ?').bind(courseId).first().catch(() => null);
    ...
    return jsonResponse({
      ...
      is_locked: Boolean(course.is_locked),
      sections: [],
      totalitems: 0
    }, 200, corsHeaders);
  }
  ...
  results.forEach(row => {
    ...
    if (!sectionsMap.has(row.section_id)) {
      sectionsMap.set(row.section_id, {
        ...
        is_locked: Boolean(row.section_locked),
        simulations: []
      });
    }
    if (row.sim_id) {
      totalItems++;
      sectionsMap.get(row.section_id).simulations.push({
        id: row.sim_id,
        slug: row.slug,
        title: row.sim_title,
        icon_class: row.sim_icon,
        is_locked: Boolean(row.sim_locked),
        has_simulation: Boolean(row.has_simulation),
  ...
  const courseData = {
    ...
    icon_class: results[0].course_icon,
    description: results[0].course_description,
    is_locked: Boolean(results[0].course_locked),
  ...
  async function handleGetSimulation(slug, env, corsHeaders) {
    const { results } = await env.DB.prepare(`
      ...
      WHERE sim.slug = ? AND sim.is_deleted = 0 AND c.is_deleted = 0
      GROUP BY sim.id
    `).bind(slug).all();
...
async function handleSearch(query, courseId, request, env, corsHeaders) {
  ...
  let sql = `
    ...
    WHERE simulations_fts MATCH ? AND sim.is_deleted = 0
  `;
  const params = [query];
...
async function handleGetRecent(limit, request, env, corsHeaders) {
  ...
  // Query D1
    const { results } = await env.DB.prepare(`
      ...
      FROM simulations sim
      JOIN courses c ON sim.course_id = c.id
      WHERE sim.is_deleted = 0 AND c.is_deleted = 0 AND c.is_active = 1
  ...
  if (!id || !title || !icon_class) {
    return jsonResponse({ error: 'Missing required fields: id, title, icon_class' }, 400, corsHeaders);
        ...
        try {
          ...
          // Robust retry for missing columns - Catch varied SQLite/D1 error messages
          const errMsg = (err.message || '').toLowerCase();
          if (errMsg.includes('no such column') || errMsg.includes('has no column named')) {
            console.warn('Missing columns, retrying basic insert:', err.message);
            try {
              await env.DB.prepare(`
              ...
              await env.KV.delete('courses:all');
              return jsonResponse({ success: true, id, warning: 'Extended fields (type/parent) not saved. Please run /api/admin/migrate' }, 201, corsHeaders);
            } catch (retryErr) {
              return jsonResponse({ error: 'Retry failed: ' + retryErr.message, originalError: err.message }, 500, corsHeaders);
            }
          }
...
async function handleAdminPermanentDeleteCourse(id, env, corsHeaders) {
  if (!id) return jsonResponse({ error: 'Missing id' }, 400, corsHeaders);
  try {
    // 1. Cascading delete simulations child data
    await env.DB.prepare('DELETE FROM statistics WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM comments WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_likes WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_dislikes WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_reports WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM bookmarks WHERE simulation_id IN (SELECT id FROM simulations WHERE course_id = ?)').bind(id).run().catch(() => {});

    // 2. Delete simulations and sections
    await env.DB.prepare('DELETE FROM simulations WHERE course_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM sections WHERE course_id = ?').bind(id).run().catch(() => {});
    ...
    await env.KV.delete('courses:all');
    await env.KV.delete(`metadata:${id}`);
    return jsonResponse({ success: true, message: 'Course and all related data permanently deleted' }, 200, corsHeaders);
  } catch (err) {
    console.error('Permanent delete course error:', err);
    return jsonResponse({ error: err.message }, 500, corsHeaders);
  }
}
  ...
  if (!id) return jsonResponse({ error: 'Missing id' }, 400, corsHeaders);
    try {
      ...
      if (errMsg.includes('no such column') || errMsg.includes('has no column named')) {
        ...
        await env.KV.delete('courses:all');
        return jsonResponse({ success: true, warning: 'parent_course_id and type not saved. Please run /api/admin/migrate' }, 200, corsHeaders);
      }
...
async function handleAdminPermanentDeleteSection(id, env, corsHeaders) {
  ...
  try {
    ...
    // 1. Cascading delete simulations child data for this section
    await env.DB.prepare('DELETE FROM statistics WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM comments WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_likes WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_dislikes WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_reports WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM bookmarks WHERE simulation_id IN (SELECT id FROM simulations WHERE section_id = ?)').bind(id).run().catch(() => {});

    // 2. Delete simulations for this section
    await env.DB.prepare('DELETE FROM simulations WHERE section_id = ?').bind(id).run().catch(() => {});
    
    // 3. Delete the section itself
    await env.DB.prepare('DELETE FROM sections WHERE id = ?').bind(id).run();
    
    if (section) await env.KV.delete(`metadata:${section.course_id}`);
    return jsonResponse({ success: true, message: 'Section and all its topics permanently deleted' }, 200, corsHeaders);
  } catch (err) { return jsonResponse({ error: err.message }, 500, corsHeaders); }
}
...
async function handleAdminPermanentDeleteSimulation(id, env, corsHeaders) {
  ...
  try {
    ...
    if (!sim) return jsonResponse({ success: true, message: 'Already deleted or not found' }, 200, corsHeaders);

    // 1. Cascading delete simulation child data
    await env.DB.prepare('DELETE FROM statistics WHERE simulation_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM comments WHERE simulation_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_likes WHERE simulation_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_dislikes WHERE simulation_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM simulation_reports WHERE simulation_id = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM bookmarks WHERE simulation_id = ?').bind(id).run().catch(() => {});

    // 2. Finally delete the simulation
    await env.DB.prepare('DELETE FROM simulations WHERE id = ?').bind(id).run();

    await env.KV.delete(`metadata:${sim.course_id}`);
    await env.KV.delete('recent:all');
    return jsonResponse({ success: true, message: 'Topic and all related data permanently deleted' }, 200, corsHeaders);
  } catch (err) { 
    console.error('Permanent delete simulation error:', err);
    return jsonResponse({ error: err.message }, 500, corsHeaders); 
  }
}
...
async function handleAdminPermanentDeleteCategory(id, env, corsHeaders) {
  if (!id) return jsonResponse({ error: 'Missing id' }, 400, corsHeaders);
  try {
    // Orphan courses in this category before deleting it
    await env.DB.prepare('UPDATE courses SET category = NULL WHERE category = ?').bind(id).run().catch(() => {});
    await env.DB.prepare('DELETE FROM categories WHERE id = ?').bind(id).run();
    return jsonResponse({ success: true }, 200, corsHeaders);
  } catch (err) { return jsonResponse({ error: err.message }, 500, corsHeaders); }
}
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
        if (type === 'courses') {
          ...
          try {
            stmts.push(
              env.DB.prepare('INSERT INTO courses (id, title, icon_class, category, display_order, is_deleted, parent_course_id, type) VALUES (?, ?, ?, ?, ?, 0, ?, ?)')
                .bind(item.id, item.title, item.icon_class, item.category, item.display_order, item.parent_course_id || null, item.type || 'course')
            );
          } catch (e) {
